...
def strain_dyad(start, end):
    ...
    with open(output[:-4] + '_' + str(start) + '-'+ str(end-1) +'.csv', 'wb') as csvtarget:
        ...
        chunk_writer.writerow(['strain_same','date_distance','cosine_sim','review_dist','type_dist','method_dist','jaccard_sim'])
        if end > rows:
            end = rows
        for i in range(start, end):
            ...
            for j in range(i+1, rows):
                ...
                if len(jaccard2.union(jaccard1)) != 0:
                    ...
                    jaccard_sim = None
                chunk_writer.writerow([strainsame,date_distance,cosine,review_distance,type_dist,method_dist,jaccard_sim])
    return
